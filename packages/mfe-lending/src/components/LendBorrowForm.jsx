...
const LendBorrowForm = ({ type, onSubmit, initialData, isEditing }) => {
  ...
  useEffect(() => {
    if (isEditing && initialData) {
      ...
      setFormData({ name: "", personName: "", amount: "", date: "", dueDate: "", notes: "", icon: "" });
    }
      ...
      <div className="flex justify-end mt-6">
        <button type="button" onClick={handleSubmit} disabled={loading} className="add-btn add-btn-fill">
          {loading ? (
            ...
            <>{isEditing ? "Update Entry" : `Save ${type === "lend" ? "Lend" : "Borrow"} Entry`}</>
          )}
        </button>
      </div>
    </div>
