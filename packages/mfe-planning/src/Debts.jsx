...
const Debts = () => {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (editDebt) {
        await axiosConfig.put(API_ENDPOINTS.UPDATE_DEBT(editDebt.id), payload);
        toast.success("Debt updated");
      } else {
        await axiosConfig.post(API_ENDPOINTS.CREATE_DEBT, payload);
  ...
  return (
    <Dashboard activeMenu="Debts">
      ...
      <Modal isOpen={showAdd} onClose={() => { setShowAdd(false); setEditDebt(null); setForm({ name: "", icon: "💳", type: "PERSONAL_LOAN", originalAmount: "", interestRate: "", emiAmount: "", startDate: "", endDate: "", notes: "" }); }} title={editDebt ? "Edit Debt" : "Add Debt"}>
        <div className="space-y-4">
          ...
          <button onClick={handleAdd} className="btn-primary w-full">{editDebt ? "Update Debt" : "Add Debt"}</button>
        </div>
      </Modal>
