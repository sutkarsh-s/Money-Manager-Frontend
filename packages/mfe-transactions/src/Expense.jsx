...
import { useUser, axiosConfig, API_ENDPOINTS, getErrorMessage, Dashboard, Modal, DeleteAlert } from "@mm/shared";
...
const Expense = () => {
    ...
    const handleUpdateExpense = async (expense) => {
        const { id, name, categoryId, amount, date, icon } = expense;

        if (!id) {
            toast.error("Expense ID is missing for update");
            return;
        }
        ...
        try {
            await axiosConfig.put(API_ENDPOINTS.UPDATE_EXPENSE(id), {
                name,
                categoryId,
